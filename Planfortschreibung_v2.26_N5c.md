# 📌 Planfortschreibung auf v2.26 — N5c-1, N5c-2 und die Klappmechanik

## Austauschblöcke für `Schweißnaht-1.md`

> **Diese Datei ersetzt `Planfortschreibung_v2_25_N5c-1_und_N5c-2.md`** — die alte bitte löschen.
>
...
Status       : N1 (Fundament), N2 (Nahtbild-Kern), N2b (Profileingabe),
               ...
               N5c-1 („Es rechnet") und N5c-2 („Es erklärt sich") am Handy
               geprüft, alles im grünen Bereich — ABGENOMMEN.
               **Nachbesserung aus der Abnahme: der Rechenweg ist jetzt
               aufklappbar (wie im Schwesterprogramm) — Abnahme steht aus.**
               → NÄCHSTER SCHRITT: erst die Klappmechanik am Handy prüfen, dann Etappe
...
Basislinie   : 822 Assertions · DOM-Smokes 448 (voll) + 449 (test) · i18n-Parität 0
...
**Klappbereiche des Rechenwegs:** `acc_weg_detail` und `acc_weg_<abschnittscode>` — sie
nutzen dieselben Ids und dieselbe Mechanik (`schalte` / `umschalten`) wie die statischen
Bereiche aus N5a, gebaut von `klappBereich()` in `ui.js`.
...
| **N5c-2** ✅ | *(abgenommen 2026-07-28)* Rechenweg mit 10 Abschnitten, Nahtbild-Grafik mit dreisprachiger Legende, die zwei Häkchenarten getrennt, Liste 2.4 sichtbar, Zahlformat je Sprache aus `rechenweg.js` | Ein vollständiger Nachweis von der Eingabe bis zur Quellenangabe |
...
- **Der Rechenweg ist jetzt aufklappbar**, wie im Schwesterprogramm DT-ProfiPassung. Grund:
  zehn Abschnitte mit über dreißig Schritten sind am Handy eine Wand zwischen Ergebnis und
  Seitenende.
- **Gebaut mit der vorhandenen Mechanik, nicht mit einer zweiten.** `ui.js` erzeugt die
  Klappbereiche jetzt mit denselben Klassen, Ids und Funktionen wie die statischen
  Formularbereiche aus N5a. Es gibt weiterhin genau **eine** Klappmechanik im Programm.
- **Was NICHT hinter die Klappe gehört — bewusst entschieden:** Die Bilanz der
  ...
  Nur die Einzelschritte klappen zu. Eine Lücke, die man erst aufklappen muss, wäre wieder
  eine stille Lücke — genau das, was 2.4 verhindern soll.
- **Zwei Ebenen:** der Detailbereich ist beim ersten Anzeigen **zu**; klappt man ihn auf,
  sind die zehn Abschnitte **offen** (niemand soll zehnmal tippen müssen), lassen sich aber
  einzeln schließen. Der Zustand überlebt Sprachwechsel und Neurechnen.
- **Keine neue CSS-Zeile nötig** — die `.acc*`-Stile aus N5a tragen das mit.
...
**v2.26 (2026-07-28):** **N5c-1 und N5c-2 am Handy abgenommen** („sieht super aus",
alles im grünen Bereich). **Nachbesserung aus der Abnahme: der Rechenweg ist aufklappbar**,
wie im Schwesterprogramm. Gebaut mit der **vorhandenen** Klappmechanik aus N5a
(`klappBereich()` erzeugt dieselben `acc*`-Ids und -Klassen, `schalte`/`umschalten`
unverändert) — es gibt weiterhin nur eine Klappmechanik und **keine neue CSS-Zeile**.
Zwei Ebenen: Detailbereich beim Start zu, die zehn Abschnitte darin offen und einzeln
schließbar; der Zustand überlebt Sprachwechsel und Neurechnen. **Bilanz der Selbstprüfung
und Liste 2.4 bleiben ohne Antippen sichtbar** — eine Lücke hinter einer Klappe wäre wieder
eine stille Lücke. Nur `ui.js`, `i18n_kern.js` und `dom_smoke_voll.js` geändert.
**Basislinie 822 Assertions unverändert · Smokes 440/441 → 448/449 · i18n-Parität 0.**
**Nächster Schritt: Klappmechanik am Handy prüfen, dann N5d.**

---

*Fortschreibung 2026-07-28 · gehört zu `Schweißnaht-1.md` · ersetzt die v2.25-Fassung*
